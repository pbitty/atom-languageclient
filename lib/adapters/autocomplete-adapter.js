...
export default class AutocompleteAdapter {
  ...
  async getSuggestions(
    ...
    if (cache && !cache.isIncomplete && cache.autoTriggeredBy === autoTriggeredBy && cache.triggerPoint === triggerPoint) {
      ...
      return autoTriggeredBy === prefixWithTrigger ? suggestions : filter(suggestions, request.prefix, {key: 'text'});
    }

    // We either don't have suggestions or they are incomplete so request from the language server
    const cancellationToken = Utils.cancelAndRefreshCancellationToken(server.connection, this._cancellationTokens);
